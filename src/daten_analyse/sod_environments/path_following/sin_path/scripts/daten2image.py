...
# 共享配置
FOLDER_PATH = '/home/qun/turtlebot3_realsensed435i/src/sod_avoidence/daten_analyse/sod_environments/path_following/sin_path'
...
def generate_trajectory_plot(obstacles):
    ...
    # 生成正弦轨迹并绘制（使用点）
    sine_trajectory = generate_sine_trajectory(0.0, 10.0, 100, 2, 1)
    x_sine, y_sine, _ = zip(*sine_trajectory)
    plt.scatter(x_sine, y_sine, color='green', s=20, label='Sine Trajectory', zorder=2)
    
    # 绘制机器人轨迹
    plt.plot(x_path, y_path, color='gold', label='Robot Trajectory', linewidth=4, zorder=3)
    
    # 绘制障碍物
    for obstacle in obstacles:
        solid_circle = Circle(obstacle[:2], obstacle[2], color="black", linewidth=4, zorder=1)
        plt.gca().add_patch(solid_circle)
        dashed_circle_safe = Circle(obstacle[:2], 0.37, color="orange", fill=False, linestyle='--', linewidth=4, zorder=1)
        plt.gca().add_patch(dashed_circle_safe)
        dashed_circle_safe = Circle(obstacle[:2], 0.4, color="black", fill=False, linestyle='--', linewidth=4, zorder=1)
        plt.gca().add_patch(dashed_circle_safe)
        dashed_circle_safe = Circle(obstacle[:2], 0.7, color="purple", fill=False, linestyle='--', linewidth=4, zorder=1)
        plt.gca().add_patch(dashed_circle_safe)
    
    # 设置坐标轴范围
    all_x = x_path + [obs[0] for obs in obstacles] + list(x_sine)
    all_y = y_path + [obs[1] for obs in obstacles] + list(y_sine)
