...
Q = np.array([[15.0, 0.0, 0.0], [0.0, 15.0, 0.0], [0.0, 0.0, 0.005]])  
R = np.array([[0.1, 0.0], [0.0, 0.01]])  
Rd = np.diag([10.0, 5.0])  
...
target_points = [
    #previous
    # np.array([3, 4, np.pi/2]).reshape(-1, 1), #one_obstacle
    # np.array([3, 3, np.pi/2]).reshape(-1, 1) #five_obstacles
    #ten_obstacles
    # np.array([3, 3, -np.pi/2]).reshape(-1, 1), 
    # np.array([3, 0, -np.pi]).reshape(-1, 1),
    # np.array([0, 0, 0]).reshape(-1, 1)
    #twenty_one_obstacles
    # np.array([3,3,-np.pi/2]).reshape(-1, 1),
    # np.array([3,0,-np.pi]).reshape(-1, 1),
    # np.array([0,3,-np.pi/2]).reshape(-1, 1),
    # np.array([0,0,0]).reshape(-1, 1)
    ####
    # np.array([4, 4, np.pi/2]).reshape(-1, 1), #1-10_obstacle
    np.array([5, 5, np.pi/2]).reshape(-1, 1),
]
    ...
    if current_target_index < len(target_points):
        ...
        if latest_obstacles:
            ...
            for obstacle in latest_obstacles:
                obs_x, obs_y, obs_r, obs_vx, obs_vy = obstacle
                ...
                if distance <= 0.4:
                    ...
                    try:
                        ...
                        if problem.status == cp.OPTIMAL:
                            ...
                            print(f"CVXPY problem not optimal. Status: {problem.status}")
                            # Use original MPC solution or implement a fallback strategy
                    except cp.error.SolverError:
                        print("CVXPY solver error occurred")
