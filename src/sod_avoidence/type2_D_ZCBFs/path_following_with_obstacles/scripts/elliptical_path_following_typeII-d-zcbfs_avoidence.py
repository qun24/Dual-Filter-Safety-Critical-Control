...
def is_obstacle_near_target(target, obstacles, threshold=0.6):
...
def process_obstacle(last_obstacle, robot_state, control_params):
    ...
    # Pre-calculations and fixed parameters
    ...
    distance = np.sqrt(delta_x_offset ** 2 + delta_y_offset ** 2) - obs_r - rob_diam/2 - 0.25
    ...
    if distance <= 0.4:
        ...
        e3 = 1.0 / np.sqrt(dist_sq) * (delta_x_offset * obs_vx + delta_y_offset * obs_vy)  # CBF for moving obstacle
...
v_max = 0.2
omega_max = np.pi / 8.0 
l = 0.02  # Camera offset from the center of the car
...
Q = np.array([[15.0, 0.0, 0.0], [0.0, 15.0, 0.0], [0.0, 0.0, 0.005]])  # Weights for x, y, theta
R = np.array([[0.1, 0.0], [0.0, 0.01]])  # Weights for linear velocity v and angular velocity w
Rd = np.diag([10.0, 5.0])  # Penalties for changes in linear and angular velocities
...
while not flag:
    if not target_points:
        ...
        Rate.sleep()
        continue
     # Check and skip path points occupied by obstacles
    ...
    if current_target_index < len(target_points):
        ...
        u_sol = ca.reshape(res['x'], n_controls, N)  # Get MPC input
        ...
        # if np.linalg.norm(initial_state[:2] - target[:2]) < 0.15:
        if np.linalg.norm(initial_state[:2] - target[:2]) < 0.2:
