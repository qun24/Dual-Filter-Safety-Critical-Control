...
def process_obstacle(last_obstacle, robot_state, control_params):
    ...
    distance = np.sqrt(delta_x_offset ** 2 + delta_y_offset ** 2) - obs_r - rob_diam/2 - 0.15
...
### target point
target_points = [
    # np.array([5, 5, np.pi/2]).reshape(-1, 1)  # mix obstacles target
    np.array([0.0, -4.5, -np.pi/2]).reshape(-1, 1), #warehouse simulation
]
    ...
    if current_target_index < len(target_points):
        ...
        if latest_obstacles:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(process_obstacle, obstacle, robot_state, control_params) for obstacle in latest_obstacles]
